...
final class TaskStore {
    ...
    // MARK: - Sample data (debug seeding)

    /// Wipes all data and inserts a fixed set of sample projects, tasks, and
    /// subtasks. Used to reproduce a fresh-user store for testing. Triggered by
    /// launching with the TT_SEED_SAMPLE=1 environment variable; never runs in
    /// normal use.
    func seedSampleData() {
        ...
        if let tasks = try? context.fetch(FetchDescriptor<Task>()) {
            for t in tasks { context.delete(t) }
        }

        // Build three projects mirroring the screenshots.
        func makeProject(_ title: String, _ desc: String) -> Project {
            let p = Project(title: title, desc: desc)
            context.insert(p)
            return p
        }

        let personal = makeProject("Personal", "Errands, health, and home")
        let side     = makeProject("Side Project", "Weekend app ideas")
        let work     = makeProject("Work", "Professional tasks and deliverables")

        // Adds a root task with optional subtasks; mirrors the real write path
        // (set the to-one side AND append to the inverse collection).
        func addRoot(_ title: String, to project: Project, priority: Int = 1,
                     done: Bool = false, subtasks: [(String, Bool)] = []) {
            let task = Task(plainTitle: title, priority: priority, project: project)
            task.setDone(done)
            context.insert(task)
            project.tasks.append(task)
            task.sortIndex = project.tasks.filter { $0.parent == nil }.count - 1
            for (i, sub) in subtasks.enumerated() {
                let s = Task(plainTitle: sub.0, project: project, parent: task)
                s.setDone(sub.1)
                s.sortIndex = i
                context.insert(s)
        ...
        addRoot("Apartment deep clean", to: personal, subtasks: [
            ("Vacuum and mop all floors", false),
            ("Organise wardrobe", false),
            ("Clean kitchen appliances", true),
        ])
        addRoot("Book dentist appointment", to: personal, priority: 0)
        addRoot("Finish reading Atomic Habits", to: personal, done: true, subtasks: [
            ("Write notes and takeaways", false),
            ("Read chapters 7–10", true),
        ])
        addRoot("Renew gym membership", to: personal, done: true)

        addRoot("Sketch app concept", to: side, priority: 0)
        addRoot("Build prototype", to: side)
        addRoot("Pick a name", to: side, priority: 2)

        addRoot("Finish quarterly report", to: work, priority: 0)
        addRoot("Reply to client emails", to: work)
        addRoot("Prepare team standup notes", to: work, priority: 2)
